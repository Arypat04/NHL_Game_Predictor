"""
NHL Model Training & Evaluation (matchup model).

Temporal train/test split on the one-row-per-game matchup dataset; reports
per-model and ensemble precision / accuracy / AUC-ROC / Brier, plus feature
importance. Feature engineering and model config are imported from the live
modules (nhl_matchup, nhl_predictor) so this always evaluates production.

Only knob here is TEST_SEASON (the held-out season label).
"""
...
from nhl_predictor import ENSEMBLE_WEIGHTS, TRAIN_CSV, _build_models

warnings.filterwarnings("ignore")

TEST_SEASON = 2025   # season label held out for testing
...
if __name__ == "__main__":
    print("=" * 64)
    print("  NHL Matchup Model — Training & Evaluation")
    print("=" * 64 + "\n")
    game, cols = load_data()
    evaluate(game, cols)
